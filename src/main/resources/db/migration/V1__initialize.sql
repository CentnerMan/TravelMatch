...
(
    ...
    value        integer NOT NULL DEFAULT 0
        CONSTRAINT language_skill_value CHECK (value > 0 AND value <= 5),
    created      timestamp,
...
(
    ...
    like_dislike integer
        CONSTRAINT like_dislike_value CHECK (like_dislike in (-1, 1)),
    rating       integer
...
(
    ...
    product_type VARCHAR(7)   NOT NULL
        CONSTRAINT advert_categories_product_type CHECK (product_type in ('PRODUCT', 'SERVICE')),
    PRIMARY KEY (id)
);
...
(
    ...
    price             bigint     NULL,
    product_type      VARCHAR(7) NOT NULL
        CONSTRAINT adverts_product_type CHECK (product_type in ('PRODUCT', 'SERVICE')),
    product_condition VARCHAR(4) NULL
        CONSTRAINT adverts_product_condition CHECK (product_condition in ('NEW', 'USED')),
    title             VARCHAR(255),
    text              VARCHAR(10000),
    type              VARCHAR(4) NOT NULL
        CONSTRAINT adverts_type CHECK (type in ('BUY', 'SALE')),
    user_id           bigint     NOT NULL,
    PRIMARY KEY (id),
...
(
    ...
    status          VARCHAR(6)   NOT NULL default 'NEW'
        CONSTRAINT advert_claims_status CHECK (status in ('CLOSED', 'NEW')),
    PRIMARY KEY (id),
